...
def main():
    ...
    loader = make_loader(train_ds, cfg["training"]["batch_size"], True, cfg["training"]["num_workers"])
    ...
    for epoch in range(cfg["training"]["epochs"]):
        ...
        print(f"epoch={epoch+1:02d} train_loss={loss:.4f} test_acc={metrics['accuracy']:.4f}")
    elapsed = time.perf_counter() - start

    out = Path(cfg["paths"]["checkpoints"])
    out.mkdir(parents=True, exist_ok=True)
    torch.save({"model_state": model.state_dict(), "config": cfg, "train_seconds": elapsed}, out / "original.pt")
